class Shape:
    ...
    def addType(self):
        ...
        Shape.s_count += 1
        self.shapeList.update({self.shape: [self._color , self.lenght, self.widht]})    #the list should be updated every time when person write new info

    
    def getPerimeter(self):
        return self.edges*(self.lenght + self.widht)
    
    def getArea(self):
        if self.shape == 1:
            self.area = self.lenght * self.widht
